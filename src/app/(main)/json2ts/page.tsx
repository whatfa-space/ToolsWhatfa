  ...
  return (
    <div className='p-6'>
      <TagContainer title='Json转typescript'>
        ...
        <textarea value={json} onChange={onInput} className='textarea textarea-bordered w-full h-40' placeholder='请输入JSON'></textarea>
        <div className='flex my-4'>
          <button onClick={handleConvert} className='btn btn-sm btn-neutral mr-2'>转换</button>
          <button className='btn btn-sm btn-neutral'>复制</button>
        </div>
        <textarea value={tsResult} className='textarea textarea-bordered w-full h-40' disabled placeholder='等待转换...'></textarea>
      </TagContainer>
    </div>
  )
}